    ...
    LaunchedEffect(formEvent) {
        when (val event = formEvent) {
            is TransactionFormEvent.Success -> {
                scope.launch { snackbarHostState.showSnackbar("Stok berhasil dikurangi") }
            ...
            if (current == null) {
                ...
                Card(modifier = Modifier.fillMaxWidth()) {
                    Column(modifier = Modifier.padding(12.dp)) {
                        ...
                        Text(text = "Stok tersedia: ${current.currentStock} ${current.unit}")
                ...
                FormTextField(label = "Tujuan / Penerima (opsional)", value = destination, onValueChange = { destination = it })
                ...
                Button(
                    onClick = {
                        ...
                        when {
                            qty <= 0 -> quantityError = "Jumlah stok harus lebih besar dari 0"
                            qty > current.currentStock -> quantityError =
                                "Stok keluar melebihi stok tersedia (${current.currentStock})"
                            else -> transactionViewModel.recordStockOut(
                                product = current,
                                quantity = qty,
